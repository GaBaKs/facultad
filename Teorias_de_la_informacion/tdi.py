    ...
    for j in range(largo):
        ...
        if suma_col > 0:
            for i in range(largo):
                matriz_prob[i][j] = round(matriz[i][j] / suma_col, 4)
...
def calculaVecEstacionario(mat):
    ...
    for elem in vec_estacionario:
        vec_estacionario[vec_estacionario.index(elem)] = round(elem,3)
...
def entropiaMarkoviana(matriz):
    ...
    return round(sum(vecEstacionario),4)
...
def codigoUnivoco(lista):
    ...
    while  set(subconjunto) != set(subconjunto_anterior) and not flag2:
        ...
        print("Subconjuto Anterior: ", subconjunto_anterior)
        for codigo in subconjunto:
            for otro_codigo in lista:
                if otro_codigo.startswith(codigo):
                    ...
                    if resto == "":
                        flag2=True
        print("Subconjuto Actual: ", subconjunto)
    return not flag2

def generaCadenaCodificada(cadena):
    """
    Obtiene los símbolos únicos (alfabeto) de la cadena, ordenados.
    """
    # Usamos set() para encontrar únicos y sorted() para ordenar
...
def calculaBaseR(alfabeto):
    # 'alfabeto' es una lista de palabras, ej: ['0100', '0101', '100']
    
    # Corregido: Unimos la lista de palabras en un solo string
    alfabeto_str = "".join(alfabeto) 
    # ej: '01000101100'
    
    # Ahora si contamos los simbolos unicos de ese string
    simbolos_unicos = generaCadenaCodificada(alfabeto_str)
    # ej: ['0', '1']
    
    return len(simbolos_unicos) # ej: 2
...
def entropiaBase2(probabilidades):
    ...
    for p in probabilidades:
        if p!=0:
            acum += p * math.log(1/p, 2)
    return acum

def cadenaADistribucion(cadena):
    """
    Calcula el alfabeto ordenado y la distribución de probabilidades.
    """
    # Obtenemos el alfabeto ordenado
    alfabeto = generaCadenaCodificada(cadena) # Llama a la versión ordenada
    
    apariciones = []
    # Contamos las apariciones para cada símbolo en el orden del alfabeto
    for simbolo in alfabeto:
        apariciones.append(cadena.count(simbolo))
        
    # Calcula la probabilidad (frecuencia relativa)
